...
local close_terminal = require("kola.toggleterm").try_close_vertical_terminal
...
bufferline.setup({
	options = {
		...
		left_mouse_command = function(buffer_number)
			local current_buffer = vim.fn.expand("%")
			if string.find(current_buffer, "toggleterm") then
				vim.cmd("wincmd h")
			end
